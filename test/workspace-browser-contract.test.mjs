...
test("advisor console presents a threshold-governed steward command center", async () => {
  ...
  assert.match(html, /Step 1 of 5 · Engagement pulse/);
  ...
  assert.match(html, /You are stewarding this POC/);
  assert.match(html, /Your five-step stewardship journey/);
  assert.match(html, /Why this matters/);
  assert.match(html, /You are finished when/);
  assert.match(html, /Engagement pulse/);
  assert.match(html, /Prepare revelation/);
  assert.match(script, /organizationLabel/);
  assert.match(script, /entitlementType/);
  assert.match(script, /function showStage/);
  assert.match(script, /function markCurrentStage/);
  assert.match(script, /function expandNarratives/);
  assert.match(styles, /\.advisor-intervention-form[^}]+grid-template-columns:\s*1fr/);
  assert.match(styles, /textarea[^}]+overflow:\s*hidden/);
  assert.match(styles, /\.advisor-stage-guide/);
...
test("workspace keeps assessment collection separate from paid diagnostic engagement", async () => {
  ...
  assert.match(html, /Build the room before choosing the people/);
  assert.match(html, /assembling the right room—not building a statistical sample/);
  assert.match(html, /Who should this represent/);
  assert.match(html, /How do they experience the work/);
  assert.match(html, /What area do they see/);
  assert.match(html, /VP, Director, Manager, Team Lead, and Sales Representative/);
  assert.match(html, /What your current cohort can reveal/);
...
test("advisor console is separate, assignment-scoped, and excludes raw interview fields", async () => {
  ...
  assert.match(html, /Guide the diagnostic from inquiry to revelation/);
  ...
  assert.match(html, /data-internal-method-control hidden/);
  assert.match(advisor, /remains undetermined until confidential multi-perspective evidence/);
  assert.match(advisor, /diagnostic-frame-v2/);
  assert.match(advisor, /current-and-anticipated/);
  assert.match(advisor, /No finding has been released/);
  assert.match(html, /Participant question set/);
  assert.match(html, /shared inquiry/);
  assert.match(html, /does not produce individual or mechanism scores/);
  assert.match(advisor, /diagnostic-protocol-v2/);
  assert.match(advisor, /evidenceLayerId/);
  assert.match(advisor, /Collection remains inactive until platform activation/);
...
test("private Executive Diagnostic Simulation is governed, fictional, and disconnected from live records", async () => {
  const html = await source("workspace/presenter-demo.html");
  const script = await source("workspace/presenter-demo.js");
  const server = await source("scripts/workspace-dev-server.mjs");
  assert.match(html, /Private simulation/);
  assert.match(html, /Executive Diagnostic Simulation/);
  ...
  assert.match(html, /data-lens="sponsor"/);
  assert.match(html, /data-lens="participant"/);
  assert.match(html, /data-lens="steward"/);
  assert.match(html, /data-pressure-options/);
  assert.match(html, /data-scope-options/);
  assert.match(html, /presenter-access-copy/);
  assert.match(html, /data-timeline/);
  assert.match(html, /data-inspector-content/);
  assert.match(html, /Presenter intelligence/);
  assert.match(script, /id:"frame"/);
  assert.match(script, /id:"design"/);
  assert.match(script, /id:"collect"/);
  assert.match(script, /id:"synthesize"/);
  assert.match(script, /id:"reveal"/);
  assert.match(script, /commercial:/);
  assert.match(script, /retention:/);
  assert.match(script, /execution:/);
  assert.match(script, /scaling:/);
  assert.match(script, /change:/);
  assert.match(script, /leadership:/);
  assert.match(script, /organization:/);
  assert.match(script, /function:/);
  assert.match(script, /crossFunctional:/);
  assert.match(script, /Executive Capacity Brief/);
  assert.match(script, /Revelation Guide/);
  assert.match(script, /Evidence convergence/);
  assert.match(script, /Counterpoint retained/);
  assert.match(script, /What would change our interpretation/);
  assert.match(script, /Decision support—not probability of success/);
  assert.match(script, /No participant is invited yet/);
  assert.match(script, /Illustrative perspectives · this fictional engagement/);
  assert.match(script, /Frontline Sales/);
  assert.match(script, /Product Manager/);
  assert.match(script, /Customer Success Director/);
  assert.match(script, /Engineering Manager/);
  assert.match(script, /Regional Operations Lead/);
  assert.match(script, /observational access—not the participant’s answer, score, or inferred finding/);
  assert.match(script, /Protected reasoning room/);
  assert.match(script, /Decision escalation/);
  assert.match(script, /Late customer intelligence/);
  assert.match(script, /Compensating coordination/);
  assert.match(script, /What strengthens it/);
  assert.match(script, /What weakens it/);
  assert.match(script, /What would change our interpretation/);
  assert.match(script, /Best current fit—not a probability of truth/);
  assert.match(script, /Protected synthesis · sponsor view/);
  assert.match(script, /Testing explanations—not summarizing opinions/);
  assert.match(script, /Visible now/);
  assert.match(script, /Protected until Revelation/);
  assert.match(script, /3 of 5 gates complete/);
  assert.match(script, /Question 6 of 18/);
  assert.match(script, /Below five completions, organizational results remain locked/);
  assert.match(script, /No raw response, attributable excerpt, or individual score/);
  assert.doesNotMatch(script, /pressure test|Pressure Test/);
  assert.doesNotMatch(script, /localStorage|sessionStorage/);
  assert.match(script, /workspace\/presenter-demo-access/);
