...
test("guided presenter demo is fictional, resettable, and disconnected from live records", async () => {
  ...
  assert.match(html, /data-role="steward"/);
  assert.match(script, /label:"Optional assessment"/);
  assert.match(script, /label:"Learn & schedule"/);
  assert.match(script, /label:"Six sponsor prompts"/);
  assert.match(script, /label:"Design perspectives"/);
  assert.match(script, /label:"Approve protocol"/);
  assert.match(script, /label:"Confidential participation"/);
  assert.match(script, /label:"Threshold & synthesis"/);
  assert.match(script, /label:"Steward governance"/);
  assert.match(script, /label:"Revelation"/);
  assert.match(script, /label:"POC completion"/);
  assert.match(script, /Executive Capacity Brief/);
  assert.match(script, /Revelation Guide/);
  assert.match(script, /30 days/);
  assert.match(script, /permanent after purchase/i);
  ...
  assert.match(script, /de-identified and disclosure-reviewed/i);
  assert.match(script, /detailed design, provider selection, coaching, training, implementation, and reassessment/i);
