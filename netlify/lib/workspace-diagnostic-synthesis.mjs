...
export async function persistDiagnosticSynthesis({ workspaceId, userId, diagnosticId, synthesis, model, now = new Date() }, connectionString) {
  return withNeonWorkspaceTransaction(workspaceId, async ({ query }) => {
    ...
    const payload = { themes: synthesis.themes, hypothesis: synthesis.hypothesis, generatedBy: { kind: "ai-assisted-advisor-draft", model } };
    const inserted = await query(
      `INSERT INTO app_shared.diagnostic_findings
       ...
       VALUES ($1, $2, '1.0.0', $3, $4::jsonb, $5, 'draft')
      ...
      [workspaceId, diagnosticId, synthesis.hypothesis.synthesisVersion, JSON.stringify(payload), synthesis.hypothesis.confidence]
    );
    ...
    await query(
      ...
      [workspaceId, userId, diagnosticId, JSON.stringify({ synthesisVersion: synthesis.hypothesis.synthesisVersion, themeCount: synthesis.themes.length, confidence: synthesis.hypothesis.confidence, model })]
    );
...
export async function createDiagnosticSynthesis({ workspaceId, userId, diagnosticId, synthesize, model, connectionString }) {
  const evidence = await loadApprovedEvidenceForSynthesis({ workspaceId, userId, diagnosticId }, connectionString);
  const synthesis = await synthesize({ diagnosticId, evidence });
