...
export async function getStewardCommandCenter({workspaceId,userId,diagnosticId,now=new Date()},connectionString){return withNeonWorkspaceTransaction(workspaceId,async({query})=>{
  ...
  const [diagnostic,context,plan,protocol,activation,sessions,slots,evidence,finding,executiveBrief,revelationGuide,validation]=await Promise.all([
  ...
  const source={sessions:sessions.rows,context:context.rows[0],plan:plan.rows[0],protocol:protocol.rows[0],activation:activation.rows[0],planned,invited:slotRows.length,completed,evidence:evidenceSummary,finding:finding.rows[0],executiveBrief:executiveBrief.rows[0],revelationGuide:revelationGuide.rows[0],validation:validation.rows[0]};
