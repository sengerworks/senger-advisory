...
async function jsonBody(request) {
  ...
  try { return JSON.parse(text); } catch { throw new DiagnosticContextInputError("Enter valid context details."); }
}

export function createWorkspaceDiagnosticContextHandler({
  authenticate = authenticateWorkspaceRequest,
  getContext = getWorkspaceDiagnosticContext,
  approveContext = approveWorkspaceDiagnosticContext
} = {}) {
  return async request => {
    ...
    try {
      ...
      if (!authorizeWorkspaceAction({ role: auth.value.role, action })) {
        return json(403, { error: "Only workspace administrators can manage diagnostic discovery." });
      }
      if (request.method === "GET") {
        const diagnosticId = validateDiagnosticContextId(url.searchParams.get("diagnosticId"));
        return json(200, {
          context: await getContext(auth.value.workspaceId, diagnosticId),
          policy: workspaceDiagnosticDiscoveryPolicy
        });
      }
      const input = validateDiagnosticContextInput(await jsonBody(request));
      return json(201, {
        context: await approveContext({
      ...
      if (error instanceof DiagnosticContextStateError) return json(409, { error: error.message });
      console.error("Workspace diagnostic context operation failed", error instanceof Error ? error.message : "Unknown error");
      return json(503, { error: "Diagnostic discovery is temporarily unavailable." });
    }
  };
}

export default createWorkspaceDiagnosticContextHandler();
export const config = { path: "/api/workspace/diagnostic-context", rateLimit: { windowLimit: 30, windowSize: 60, aggregateBy: ["ip", "domain"] } };
